  ...
  Future<void> _loadUsers({String? search}) async {
    ...
    try {
      ...
      final uniqueUsers = _dedupeUsersById(users)
        ..sort((a, b) {
          final aPinned = a.getBoolValue('is_pinned');
          final bPinned = b.getBoolValue('is_pinned');
          if (aPinned != bPinned) return aPinned ? -1 : 1;
          if (aPinned && bPinned) {
            final aAt = DateTime.tryParse(a.getStringValue('pinned_at'));
            final bAt = DateTime.tryParse(b.getStringValue('pinned_at'));
            if (aAt != null && bAt != null) return bAt.compareTo(aAt);
            if (aAt != null) return -1;
            if (bAt != null) return 1;
          }
          final aCreated = DateTime.tryParse(a.getStringValue('created_at'));
          final bCreated = DateTime.tryParse(b.getStringValue('created_at'));
          if (aCreated != null && bCreated != null) {
            return bCreated.compareTo(aCreated);
          }
          return a.getStringValue('name').compareTo(
                b.getStringValue('name'),
              );
        });
      setState(() {
        _users = uniqueUsers;
  ...
  Future<void> _loadCustomerInboxInBackground(
    ...
    try {
      ...
      sorted.sort((a, b) {
        ...
        if (aPinned && bPinned) {
          final aPinnedAt =
              DateTime.tryParse(a.getStringValue('pinned_at'));
          final bPinnedAt =
              DateTime.tryParse(b.getStringValue('pinned_at'));
