...
def patch_dashboard():
    ...
    text = path.read_text()
    start = text.index('  Widget _buildNavItem(')
    end = text.index('  Widget _buildNewDashboard(', start)
    replacement = '''  Widget _buildNavItem(
    int index,
    IconData icon,
    IconData activeIcon,
    String label,
...
def patch_user_list():
    ...
    text = path.read_text()
    start = text.index('  Widget _buildUserCard(')
    end = text.index('  List<Color> _getAvatarGradient(', start)
    replacement = '''  Widget _buildUserCard(RecordModel user, int index, AuthProvider auth) {
    final isDark = Theme.of(context).brightness == Brightness.dark;
