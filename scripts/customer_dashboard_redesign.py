...
path = Path('lib/screens/customer/customer_dashboard.dart')
text = path.read_text()

text = text.replace("import 'package:zhirox/providers/theme_provider.dart';\n", "")
...
new_load = r'''  Future<void> _loadDebts() async {
...
text, count = re.subn(
    r"  Future<void> _loadDebts\(\) async \{.*?\n  \}\n\n  Future<void> _loadMoreHistory",
    new_load + "\n  Future<void> _loadMoreHistory",
    text,
    count=1,
    flags=re.S,
...
if count != 1:
    raise SystemExit('Could not replace _loadDebts')
...
if count != 1:
    raise SystemExit('Could not replace dashboard build tail')
