-- Shared database contract for owner-source and user-source.
-- owner-source only manages can_import_data; import execution remains in user-source.
...
begin
  ...
  from public.debts d
  where d.id = p_debt_id and coalesce(d.is_deleted, false) = false
  for update;
  if not found then raise exception 'debt_not_found'; end if;
  select p.admin_id into v_customer_admin from public.profiles p where p.id = v_debt.customer_id;
  if v_customer_admin is distinct from p_admin_id then raise exception 'cross_tenant_forbidden'; end if;
  if exists(select 1 from public.payments where id = p_payment_id) then return; end if;
  if p_amount > v_debt.remaining then raise exception 'payment_exceeds_remaining'; end if;
...
begin
  if not exists (
    ...
    where id = auth.uid() and is_system_owner = true and active = true
  ) then
    raise exception 'system_owner_required';
  end if;
  update public.profiles
  set can_import_data = coalesce(p_allowed, false), updated_at = now()
