-- Add server-side filters and summary counters to the manager push overview.
...
begin
  if v_filter not in ('all', 'active', 'inactive', 'failed', 'pending') then
    raise exception 'invalid_push_overview_filter' using errcode = '22023';
  end if;
  ...
  filtered as (
    ...
    from base
    where v_filter = 'all'
       or (v_filter = 'active' and device_count > 0)
       or (v_filter = 'inactive' and device_count = 0)
       or (v_filter = 'failed' and latest_status in ('failed', 'partial'))
       or (v_filter = 'pending' and latest_status = 'pending')
  ),
  paged as (
    select *
    from filtered
    ...
    coalesce(
      (select jsonb_agg(
        jsonb_build_object(
          ...
          'name', p.name,
          'phone', p.phone,
          'active', p.device_count > 0,
          'device_count', p.device_count,
          'active_link_count', p.active_link_count,
          'latest_status', p.latest_status,
          'latest_at', p.latest_at
        )
        order by p.name, p.customer_id
      ) from paged p),
      '[]'::jsonb
    ),
    (select count(*)::integer from filtered),
    (select count(*)::integer from base),
    (select count(*)::integer from base where device_count > 0),
    (select count(*)::integer from base where device_count = 0),
    (select count(*)::integer from base where latest_status in ('failed', 'partial')),
    (select count(*)::integer from base where latest_status = 'pending')
  into
    v_rows,
    v_total,
    v_all_count,
...
set search_path = ''
as $$
  select public.list_customer_push_overview_service(
    p_actor,
    p_search,
    'all',
    p_limit,
...
revoke all on function public.list_customer_push_overview_service(uuid,text,text,integer,integer)
  from public, anon, authenticated;
revoke all on function public.list_customer_push_overview_service(uuid,text,integer,integer)
  from public, anon, authenticated;

grant execute on function public.list_customer_push_overview_service(uuid,text,text,integer,integer)
  to service_role;
