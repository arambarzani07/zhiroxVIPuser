-- Reconciliation recognizes customer-wide/general payment mappings.
            ...
            where p.sync_source_id=v_source.id
              ...
              and (
                ...
                or exists (
                  ...
                  left join public.payments pay on pay.id=a.target_id
                  left join public.customer_general_payments gp
                    on gp.id=a.target_id and gp.admin_id=v_source.admin_id
                  where a.sync_source_id=v_source.id
                    and a.entity_kind='payment_allocation'
                    and a.source_id like t.source_id || ':%'
                    and (pay.id is not null or gp.id is not null)
                )
              )
          )
          or exists (
            ...
            left join public.payments pay on pay.id=l.target_id
            left join public.customer_general_payments gp
              on gp.id=l.target_id and gp.admin_id=v_source.admin_id
