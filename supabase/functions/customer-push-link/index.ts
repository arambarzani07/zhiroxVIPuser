...
function portalHtml(token: string): string {
  const manifestSuffix = isToken(token) ? `?token=${encodeURIComponent(token)}` : "";
  return `<!doctype html>
...
<head>
  ...
  <meta name="apple-mobile-web-app-title" content="ZHIROX">
  <meta name="description" content="پۆرتالی ڕەسمی کڕیار بۆ بینینی قەرز، پارەدان و ئاگادارکردنەوەکان">
  <title>ZHIROX Customer Portal</title>
  <link rel="manifest" href="${CUSTOMER_PUSH_LINK_BASE_PATH}/manifest.webmanifest${manifestSuffix}">
  <link rel="stylesheet" href="${CUSTOMER_PUSH_LINK_BASE_PATH}/styles.css">
  <script defer src="${CUSTOMER_PUSH_LINK_BASE_PATH}/app.js"></script>
</head>
<body>
  <main class="portal-shell">
    <header class="portal-header">
      <div class="brand-lockup">
        ...
        <div>
          <p class="eyebrow">پۆرتالی ڕەسمی کڕیار</p>
          <h1 id="marketBrand">ZHIROX</h1>
          <p id="customerGreeting" class="muted">بارکردنی زانیاریی هەژمار...</p>
        </div>
      </div>
      <div id="accountBadge" class="status-badge" role="status">پشکنین...</div>
    </header>
    ...
    <section id="lockedState" class="state-card" hidden>
      ...
      <p class="eyebrow">پاراستنی هەژمار</p>
      <h2>دەستگەیشتن بەردەست نییە</h2>
      <p class="muted">ئەم لینکە بەردەست نییە یان ڕاگیراوە. QR ـێکی چالاک لە سوپەرمارکێتەکە وەربگرە.</p>
    </section>

    <div id="portalApp" hidden>
      <nav class="portal-nav" aria-label="بەشەکانی پۆرتال">
        <button id="homeTab" type="button" class="nav-item is-active" data-tab="home">سەرەکی</button>
        <button id="transactionsTab" type="button" class="nav-item" data-tab="transactions">مامەڵەکان</button>
        <button id="notificationsTab" type="button" class="nav-item" data-tab="notifications">ئاگادارکردنەوە</button>
      </nav>

      <section id="homeView" class="portal-view">
        <div class="view-heading"><div><p class="eyebrow">پوختەی هەژمار</p><h2>دۆخی دارایی</h2></div><span class="powered-by">Powered by ZHIROX</span></div>
        <article class="balance-card"><div class="balance-topline"><span>قەرزی ماوە</span><span class="balance-status">نوێکراوە</span></div><strong id="primaryRemaining">0 IQD</strong><span id="primaryCurrency" class="balance-caption">IQD</span></article>
        <div id="summaryMetrics" class="summary-grid"></div>
        <section class="panel"><div class="section-heading"><div><p class="eyebrow">دوایین چالاکی</p><h2>مامەڵە نوێیەکان</h2></div><button id="openTransactions" class="text-action" type="button">هەمووی ببینە</button></div><div id="recentLedger" class="ledger"></div></section>
      </section>

      <section id="transactionsView" class="portal-view" hidden>
        <div class="view-heading"><div><p class="eyebrow">کەشفی حیساب</p><h2>قەرز و پارەدانەکان</h2></div></div>
        <section class="panel"><div id="ledger" class="ledger"></div><button id="loadMore" class="secondary-action" type="button" hidden>بینینی مامەڵەی زیاتر</button></section>
      </section>

      <section id="notificationsView" class="portal-view" hidden>
        <div class="view-heading"><div><p class="eyebrow">Web Push</p><h2>ئاگادارکردنەوەکان</h2></div></div>
        <article class="notification-card">
          <p class="eyebrow">دۆخی ئاگادارکردنەوە</p>
          <div id="notificationState" class="notification-state">پشکنین...</div>
          <p class="muted notification-copy">ئاگادارکردنەوەکانی قەرز، پارەدان و پەیامی بەڕێوەبەر بە ناوی سوپەرمارکێتەکەت دەنێردرێن.</p>
          ...
          <p id="result" class="action-result" aria-live="polite"></p>
        </article>
        <section class="panel"><p class="eyebrow">پاراستن</p><h2>زانیاریی هەژمارەکەت پارێزراوە</h2><p class="muted">لینک و ناسنامەی ئامێر لە پۆرتالەکەدا پیشان نادرێن. بەڕێوەبەر دەتوانێت دەستگەیشتن ڕابگرێت.</p></section>
      </section>
    </div>

    <dialog id="iosHelpDialog" class="help-dialog">
      <form method="dialog">
        <button class="dialog-close" value="cancel" aria-label="داخستن">×</button>
        <p class="eyebrow">iPhone / iPad</p>
        <h2>Add to Home Screen</h2>
        <p class="muted">بۆ Web Push لە iPhone، پۆرتالەکە لە Safari بکەرەوە، Share → Add to Home Screen، پاشان لە Home Screen بیکەرەوە.</p>
        <button class="primary-action" value="default">تێگەیشتم</button>
      </form>
    </dialog>
  </main>
</body>
