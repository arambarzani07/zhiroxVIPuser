...
`;
  return response(script, 200, "application/javascript; charset=utf-8", {
...
<link rel="manifest" href="${basePath}/manifest.webmanifest?token=${safeToken}">
<meta name="theme-color" content="#ffffff">
<style>
body{font-family:system-ui,-apple-system,sans-serif;background:#f6f7fb;color:#171b2e;margin:0;min-height:100vh;display:grid;place-items:center}.card{width:min(92vw,540px);background:#fff;border-radius:22px;box-shadow:0 16px 50px #0001;padding:26px;box-sizing:border-box}.muted{color:#687086}.hidden{display:none}.row{padding:12px 0;border-bottom:1px solid #e2e5ef}button{width:100%;border:0;border-radius:14px;padding:14px 16px;font-weight:700;font-size:16px;background:#3157e0;color:#fff}button:disabled{opacity:.55}.ok{color:#0e9f6e}.err{color:#e5484d}code{direction:ltr}
</style>
</head>
<body><main class="card">
<h1>ئاگادارکردنەوەی ZHIROX</h1>
<p id="status" class="muted">پشکنینی لینک...</p>
<section id="identity" class="hidden"><div class="row"><strong>کڕیار:</strong> <span id="customerName"></span></div><div class="row"><strong>مارکێت:</strong> <span id="marketName"></span></div></section>
<div id="iosHelp" class="hidden"><p>لە iPhone/iPad: لە Safari دوگمەی Share بکە، <strong>Add to Home Screen</strong> هەڵبژێرە، پاشان ZHIROX Notifications لە Home Screen بکەرەوە.</p></div>
<button id="enable" class="hidden">چالاککردنی ئاگادارکردنەوە</button>
<p id="result"></p>
</main>
<script>
const TOKEN=${JSON.stringify(safeToken)};
const BASE=${JSON.stringify(basePath)};
const statusEl=document.getElementById('status'), identity=document.getElementById('identity'), enable=document.getElementById('enable'), result=document.getElementById('result');
function b64ToUint8(value){const pad='='.repeat((4-value.length%4)%4);const raw=atob((value+pad).replace(/-/g,'+').replace(/_/g,'/'));return Uint8Array.from([...raw].map(c=>c.charCodeAt(0)));}
function platform(){const ua=navigator.userAgent||'';if(/iPad|iPhone|iPod/.test(ua))return 'ios';if(/Android/i.test(ua))return 'android';return 'desktop';}
function standalone(){return window.navigator.standalone===true||window.matchMedia('(display-mode: standalone)').matches;}
async function api(payload){const r=await fetch(BASE,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(payload)});const data=await r.json().catch(()=>({}));if(!r.ok)throw new Error(data.error||'request_failed');return data;}
let vapid='';
(async()=>{try{const data=await api({action:'validate',token:TOKEN});document.getElementById('customerName').textContent=data.customer_name||'';document.getElementById('marketName').textContent=data.market_name||'';identity.classList.remove('hidden');vapid=data.vapid_public_key||'';const isiOS=platform()==='ios';if(isiOS&&!standalone()){document.getElementById('iosHelp').classList.remove('hidden');statusEl.textContent='سەرەتا زیادیکە بۆ Home Screen.';}else{enable.classList.remove('hidden');statusEl.textContent='لینکەکە دروستە. دەتوانیت ئاگادارکردنەوە چالاک بکەیت.';}}catch(e){statusEl.textContent='ئەم لینکە بەردەست نییە یان ماوەکەی تەواو بووە.';statusEl.className='err';}})();
...
export async function routeCustomerPush(
  req: Request,
  deps: PublicPushDeps,
): Promise<Response> {
  ...
  if (req.method === "GET" && url.pathname.endsWith("/manifest.webmanifest")) {
    ...
    if (!isToken(token)) return json({ error: "link_unavailable" }, 404);
  ...
  if (action === "validate") {
    try {
      ...
      return json({
        customer_name: inspected.customer_name ?? "",
        market_name: inspected.market_name ?? "",
  ...
  if (action === "subscribe") {
    ...
    try {
      await deps.redeem({
        tokenHash: await deps.hash(body.token as string),
        endpoint,
        p256dh,
        auth,
        deviceSecretHash: await deps.hash(rawDeviceSecret),
  ...
  if (action === "unsubscribe") {
    ...
    const changed = await deps.unsubscribe(endpoint, await deps.hash(deviceSecret));
    return json({ unlinked: changed });
  }
...
async function serve(req: Request): Promise<Response> {
  ...
  const rateLimitSalt = Deno.env.get("CUSTOMER_PUSH_RATE_LIMIT_SALT") ?? "";
  if (!url || !secret || !vapidPublicKey || !rateLimitSalt) {
  ...
  return await routeCustomerPush(req, {
    ...
    redeem: async (args) => {
      const { data, error } = await admin.rpc("redeem_customer_push_subscription_service", {
    ...
    unsubscribe: async (endpoint, deviceSecretHash) => {
      const { data, error } = await admin.rpc("unsubscribe_customer_push_subscription_service", {
        p_endpoint: endpoint,
    ...
    consumeRateLimit: async (keyHash, limit, windowSeconds) => {
      const { data, error } = await admin.rpc("consume_customer_push_rate_limit", {
        p_key_hash: keyHash,
        p_limit: limit,
